...
function FavoriteBooks() {
  ...
  const fetchData = () => {
    axios.get(url1).then((response) => {
      ...
      if (userData.booksLiked && userData.booksLiked.length > 0) {
        fetchBooks(userData.booksLiked);
      } else {
        setFavoriteBooks([]);
      }
    });
  ...
  const fetchBooks = (favoriteBookIds) => {
    // Example URL for fetching favorite books based on IDs
    ...
    axios.get(url2).then((response) => {
      ...
      const favoriteBooksData = books.filter((book) =>
        favoriteBookIds.includes(book.rank)
      );
        ...
        <div className="container mx-auto py-8">
          ...
          <div className="flex justify-center items-center my-10 gap-10 md:gap-20 flex-wrap md:px-10">
            {favoriteBooks.length > 0 ? (
              ...
              <p className="text-lg text-center">No favorite books found.</p>
            )}
          </div>
        </div>
      </div>
